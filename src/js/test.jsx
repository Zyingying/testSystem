...
class Test extends React.Component {
  ...
  componentWillMount() {
    let {testId, title, examTime} = this.props.location.query, restTime;
    let reg = /^([0-9]+)(\w+)$/;
    let finialTime = reg.exec(examTime);
    let num = finialTime[1], timeUnit = finialTime[2];
    if (timeUnit == 'min') {
      restTime = num * 60;
  ...
  submit() {
    ...
    for (i=0; i < testList.length; i++) {
      ...
      scoreCount += testList[i].score;
    }
    for (i = 0; i < answer.length; i++) {
      if (answer[i].select !== this.state.value[i]) {
        scoreCount -= testList[i].score;
      }else{
        rightCount ++;
      }
    }
    this.setState({answer: answer, list: true, scoreCount: scoreCount,rightCount:rightCount})
  ...
  render() {
    ...
    let {count, answer, value, list, scoreCount,rightCount} = this.state;
        ...
        <div className="testMain">
          ...
          <div className="answerList">
            ...
            {answer.map((ans, n) => {
              ...
              return <div className={"w-smaillList " + className} key={n}>{n + 1}</div>
            })}
            <hr/>
            <br/>
            <div>试卷：{title}</div>
            <h2>分数{scoreCount}</h2>
            <div>正确题数： {rightCount}/{testNum}</div>
            {/*<div>用时：{this.countUseTime(useTime)}</div>*/}
          </div>
          }
